...
class ViewController: UIViewController, ARSCNViewDelegate, ARSessionDelegate, UIDocumentPickerDelegate {
        ...
        if let vc = presentingViewController as? FirstViewController {
          //before dismissing the Form ViewController, pass the data inside the closure
            dismiss(animated: true, completion: { [self] in
                vc.sendValues(length, width, height)
            })
        }
    }
    ...
    @IBAction func restartButtonTapped(_ sender: Any) {
        if let scan = scan, scan.boundingBoxExists {
            ...
            let message = "Discard the current measurements and start over?"
    ...
    func backFromBackground() {
        if state == .scanning {
            ...
            let message = "The measurement was interrupted. It is recommended to restart the measurement."
            let buttonTitle = "Restart Measurement"
