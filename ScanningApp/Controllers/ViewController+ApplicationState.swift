...
extension ViewController {
    ...
    func scanningStateChanged(_ notification: Notification) {
        ...
        DispatchQueue.main.async {
            switch scanState {
            case .ready:
                print("State: Ready to measure")
                self.setNavigationBarTitle("Ready to measure")
                self.showBackButton(false)
                self.nextButton.setTitle("Next", for: [])
                if scan.ghostBoundingBoxExists {
                    self.displayInstruction(Message("Tap 'Next' to create an approximate bounding box around the object you want to dimension."))
                    self.nextButton.isEnabled = true
                } else {
                    self.displayInstruction(Message("Point at a nearby object to measure."))
            ...
            case .defineBoundingBox:
                ...
                self.nextButton.setTitle("Confirm", for: [])
            /*case .scanning:
            ...
            case .done:
                ...
                self.displayInstruction(Message("Tap 'Done' to save box dimensions and exit measure screen"))
                self.setNavigationBarTitle("Dimensioning Complete")
                self.showBackButton(true)
                self.nextButton.isEnabled = scan.boundingBoxExists
